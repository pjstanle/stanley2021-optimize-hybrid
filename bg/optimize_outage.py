...
if __name__=="__main__":
    ...
    save_filename = "battery_sweep.txt"
    ...
    battery_costs = np.array([0.1,0.2,0.3,0.4],dtype=int)

    for k in range(len(battery_costs)):

        battery_cost_multiplier = battery_costs[k]
        battery_capex = scipy.interpolate.interp1d(capex_size, cost*battery_cost_multiplier, kind='cubic')
        ...
        file = open('%s'%save_filename, 'a')
        file.write("outage hours: " + "%s"%outage_duration + '\n')
