...
if __name__=="__main__":
    ...
    nopts = 3
    solar_cost_multipliers = [1.0]

    for h in range(len(solar_cost_multipliers)):
        best_answer = 1E20
        for P in range(nopts):
            ...
            solar_cost_multiplier = solar_cost_multipliers[h]
            ...
            # print("SOLAR COST MULTIPLIER: ", solar_cost_multiplier)
            # print("COE: ", function_value)
            ...
            file = open('results/%s_%s_COE.txt'%(save_str,solar_cost_multiplier), 'a')
            file.write('%s'%(function_value) + '\n')
            file.close()

            file = open('results/%s_%s_x.txt'%(save_str,solar_cost_multiplier), 'a')
            file.write('%s'%(repr(turbine_x)) + '\n')
            file.close()

            file = open('results/%s_%s_y.txt'%(save_str,solar_cost_multiplier), 'a')
            file.write('%s'%(repr(turbine_y)) + '\n')
            file.close()

            file = open('results/%s_%s_wind.txt'%(save_str,solar_cost_multiplier), 'a')
            file.write('%s'%(len(turbine_x)*turbine_rating) + '\n')
            file.close()

            file = open('results/%s_%s_solar.txt'%(save_str,solar_cost_multiplier), 'a')
            file.write('%s'%(solar_capacity_kw) + '\n')
            file.close()

            file = open('results/%s_%s_battery.txt'%(save_str,solar_cost_multiplier), 'a')
            file.write('%s'%(battery_capacity) + '\n')
            file.close()
