...
class PlaceSolar():
    ...
    def place_solar(self):
        self.reset()
        turbine_x = self.turbine_locs[:,0]
        turbine_y = self.turbine_locs[:,1]
        ...
        if self.nturbs > 0:
            ...
            for i in range(self.nsolar_cells):
                ...
                plot_poly(self.solar_geometry, ax=ax, color="C1")
                plot_turbines(turbine_x, turbine_y, 50.0, ax=ax)
                plt.axis("square")
                # plt.axis("off")
                ax.set_yticklabels([])
                ax.set_xticklabels([])
                ...
                # if i<10:
                #     plt.savefig("frames2/frame000%s.png"%i)
                # elif i<100:
                #     plt.savefig("frames2/frame00%s.png"%i)
                # elif i<1000:
                #     plt.savefig("frames2/frame0%s.png"%i)
                # else:
                #     plt.savefig("frames2/frame%s.png"%i)
